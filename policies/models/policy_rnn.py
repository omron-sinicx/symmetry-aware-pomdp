...
class ModelFreeOffPolicy_Separate_RNN(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.actor_aux_weight = actor_aux_weight
    ...
    def forward(self, actions, rewards, observs, dones, masks, expert_masks, weights=None):
        ...
        ### 1. Critic loss
        if self.critic_type in ['aug-aux', 'aug', 'aux']:
            rotated_observs, rotated_actions, num_augments = \
            self.group_helper.transform_obss_acts(observs, actions)

            observs = torch.cat((observs, rotated_observs), dim=1)
            actions = torch.cat((actions, rotated_actions), dim=1)
            rewards = torch.cat([rewards]*(num_augments + 1), dim=1)
            dones = torch.cat([dones]*(num_augments + 1), dim=1)
            masks = torch.cat([masks]*(num_augments + 1), dim=1)
        ...
        sub_masks = masks[:, :batch_size, :]
        sub_num_valid = torch.clamp(sub_masks.sum(), min=1.0)
        sub_q1_pred = q1_pred[:, :batch_size, :]
        sub_q2_pred = q2_pred[:, :batch_size, :]
        sub_q_target = q_target[:, :batch_size, :]

        if self.critic_type in ['aug-aux', 'aux']:
            first_q1_pred, *other_q1_preds = q1_pred.chunk(num_augments + 1, dim=1)
            first_q2_pred, *other_q2_preds = q2_pred.chunk(num_augments + 1, dim=1)

            for i in range(len(other_q1_preds)):
                other_q1_preds[i] = other_q1_preds[i] * sub_masks
                other_q2_preds[i] = other_q2_preds[i] * sub_masks
            first_q1_pred = first_q1_pred * sub_masks
            first_q2_pred = first_q2_pred * sub_masks

        # masked Bellman error: masks (T,B,1) ignore the invalid error
        # this is not equal to masks * q1_pred, cuz the denominator in mean()
        # 	should depend on masks > 0.0, not a constant B*T
        if self.critic_type == 'aux':
            q1_pred = sub_q1_pred * sub_masks
            q2_pred = sub_q2_pred * sub_masks
            q_target = sub_q_target * sub_masks
            num_valid = torch.clamp(sub_masks.sum(), min=1.0)
        else:
        ...
        outputs["qf1_loss"] = qf1_loss.item()
        outputs["qf2_loss"] = qf2_loss.item()

        # symmetry critic loss
        if self.critic_type in ['aug-aux', 'aux']:
            for i, (other_q1_pred, other_q2_pred) in enumerate(zip(other_q1_preds, other_q2_preds)):
                if i == 0:
                    q_invariant_loss = ((other_q1_pred - first_q1_pred) ** 2).sum() / sub_num_valid
                else:
                    q_invariant_loss += ((other_q1_pred - first_q1_pred) ** 2).sum() / sub_num_valid
                q_invariant_loss += ((other_q2_pred - first_q2_pred) ** 2).sum() / sub_num_valid

            qf1_loss += q_invariant_loss / num_augments * self.critic_aux_weight

            outputs["critic_invariant_loss"] = (q_invariant_loss / num_augments * self.critic_aux_weight).item()

        with torch.no_grad():
            abs_td_errors = 0.5 * (torch.abs(sub_q1_pred - sub_q_target) +
                                   torch.abs(sub_q2_pred - sub_q_target))
            avg_abs_td_errors = abs_td_errors.sum(dim=0) / sub_masks.sum(dim=0)

            outputs["avg_abs_td_errors"] = avg_abs_td_errors
        ...
        ### 2. Actor loss
        policy_loss, log_probs, means = self.algo.actor_loss(
            markov_actor=self.Markov_Actor,
            markov_critic=self.Markov_Critic,
            actor=self.actor,
            actor_target=self.actor_target,
        ...
        # masked policy_loss
        if self.actor_type in ['aux']:
            policy_loss = (policy_loss[:, :batch_size, :] * sub_masks).sum() / sub_num_valid
        else:
            policy_loss = (policy_loss * masks).sum() / num_valid

        outputs["policy_loss"] = policy_loss.item()

        if self.actor_type in ['aug-aux', 'aux']:
            first_mean, *other_means = means.chunk(num_augments + 1, dim=1)

            for i in range(len(other_means)):
                other_means[i] = other_means[i][:-1] * sub_masks
            
            first_mean = first_mean[:-1] * sub_masks

            for i, other_mean in enumerate(other_means):
                self.group_helper.transform_other_means(other_mean, i)
                if i == 0:
                    actor_equivariant_loss = ((other_mean - first_mean) ** 2).sum() / sub_num_valid
                else:
                    actor_equivariant_loss += ((other_mean - first_mean) ** 2).sum() / sub_num_valid

            policy_loss += actor_equivariant_loss / num_augments * self.actor_aux_weight

            outputs["actor_equivariant_loss"] = (actor_equivariant_loss / num_augments * self.actor_aux_weight).item()
