...
SUPPORTED_IK_ROBOTS = {"SoftUR5eSquare", "SoftUR5eSquareOld", "SoftUR5eTriangle",
                       "SoftUR5ePentagon", "SoftUR5eHexagon", "SoftUR5eRound"}
...
class InverseKinematicsController(JointVelocityController):
    ...
    def setup_inverse_kinematics(self, load_urdf=True):
        ...
        # get paths to urdfs
        if 'softur5e' in self.robot_name.lower():
            self.robot_urdf = pjoin(
                os.path.join(robosuite.models.assets_root, "bullet_data"),
                "softur5e_description/urdf/softur5e_arm.urdf"
            )
        else:
            self.robot_urdf = pjoin(
                os.path.join(robosuite.models.assets_root, "bullet_data"),
